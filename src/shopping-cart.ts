...
function getArticleID(event: any) {
  return event.target.parentElement.previousElementSibling.childNodes[1]
    .innerHTML; // gibt die ID vom jeweiligen Produkt
}
...
function addEventListenersToRemoveBtn() {
  const cartArticleRemoveBtns = document.querySelectorAll(
    ".cartArticle-removebtn"
  ) as NodeListOf<HTMLButtonElement>;
  cartArticleRemoveBtns[cartArticleRemoveBtns.length - 1].addEventListener(
    "click",
    function (event) {
...
function resetCartButton(articleID: string) {
  ...
  if (checker === false) {
    for (let i = innerShopContainer.childNodes.length - 1; i >= 1; i--) {
      let actualArticleID =
        innerShopContainer.childNodes[i].childNodes[3].childNodes[1]
          .textContent;
      if (actualArticleID === articleID) {
        innerShopContainer.childNodes[
          i
        ].childNodes[5].childNodes[3].textContent = "In den Warenkorb";
      }
    }
  }
}
